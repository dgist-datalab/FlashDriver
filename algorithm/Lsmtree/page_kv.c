...
uint8_t gc_data_issue_header(struct gc_node *g, gc_params *params, int req_size){
	...
	//printf("test cnt:%d\n",cnt++);
//retry:
	result=lsm_find_run(g->lpa,&now,NULL,&found,&found_ppa,&params->level,&params->run, &params->target_level_lock);
	if(result==FOUND){
	...
	switch(result){
		...
		case FOUND:
			...
			break;
		case NOTFOUND:
			result=lsm_find_run(g->lpa,&now,NULL,&found,&found_ppa,&params->level,&params->run, NULL);
			LSM.lop->print_level_summary();
